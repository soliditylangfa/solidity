...
string YulUtilFunctions::copyLiteralToMemoryFunction(string const& _literal)
{
	...
	return m_functionCollector.createFunction(functionName, [&]() {
		return Whiskers(R"(
			...
			("storeLiteralInMem", storeLiteralToMemoryFunction(_literal))
			.render();
	});
}

string YulUtilFunctions::storeLiteralToMemoryFunction(string const& _literal)
{
	string functionName = "store_literal_to_memory_" + util::toHex(util::keccak256(_literal).asBytes());
